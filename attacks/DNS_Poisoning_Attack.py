...
class DNS:
    ...
    def checkInterface(self,accessMethod,server_ip,host,configDirectory,interface_config,running_config):
        ...
        self.configDirectory = configDirectory
        if isinstance(accessMethod,Telnet):
            return self.checkInterfaceByTelnet(accessMethod,interface_config,running_config)
        elif isinstance(accessMethod,SshVersionII):
            return self.checkInterfaceBySSH(accessMethod,interface_config,running_config)

    def solveInterfaceByTelnet(self,telnet,interface,running_config):
        if re.search("ip access-group ([^in]*)",interface,re.MULTILINE)==None:
            telnet.execute("conf t")
    ...
    def solveInterfaceBySSH(self,ssh,interface,running_config):
        print()

    def solveInterface(self,accessMethod,interface,running_config):
        if isinstance(accessMethod,Telnet):
            self.solveInterfaceByTelnet(accessMethod,interface,running_config)
        elif isinstance(accessMethod,SshVersionII):
            self.solveInterfaceBySSH(accessMethod,interface,running_config)
